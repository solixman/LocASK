...
export class UsersService {
  ...
  async getUserById(userId: string): Promise<EnrichedUser | null> {
    const usersMap = await this.getUsersByIds([userId]);
    return usersMap[userId] ?? null;
  }

  async enrichUsers<T extends { userId: string }>(
    items: T[],
  ): Promise<Array<T & { user: EnrichedUser | null }>> {
    const userIds = items.map((item) => item.userId).filter(Boolean);
    const usersMap = await this.getUsersByIds(userIds);

    return items.map((item) => ({
      ...item,
      user: usersMap[item.userId] ?? null,
    }));
  }
}