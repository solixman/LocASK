...
export default function Navbar({ user, onLoginClick, onLogout, onAskClick }: NavbarProps) {
  return (
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          {user && (
            <button 
              onClick={onAskClick}
              className="flex items-center gap-2 bg-purple-600 text-white px-4 sm:px-5 py-2.5 rounded-2xl text-sm font-black transition-all hover:bg-purple-700 active:scale-95 shadow-lg shadow-purple-500/20 mr-1 sm:mr-2"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
              </svg>
              <span className="hidden xs:inline">Ask locally</span>
            </button>
          )}
